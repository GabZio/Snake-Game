document.addEventListener('DOMContentLoaded', () => {
  ...
  function gameLoop() {
    ...
    if (head.x === food.x && head.y === food.y) {
      ...
      score++; eatSound.play(); flashEffect();
      scoreDisplay.textContent = `🍰 ${score}`;
      toNextLevelDisplay.textContent = `⏳ ${5 - (score % 5)} para siguiente`;
      if (score % 5 === 0) {
        level++; levelDisplay.textContent = `🧱 Nivel: ${level}`;
    ...
    if (head.x < 0 || head.x >= tileCountX || head.y < 0 || head.y >= tileCountY ||
      ...
      gameOverSound.play(); alert("¡Game Over!");
      recordScore(score);
      isRunning = false; bgMusic.pause(); bgMusic.currentTime = 0;
  ...
  function draw() {
    ...
    snake.forEach((seg, idx) => {
      if (idx === 0) {
        ctx.drawImage(new Image().src="snake_head_nobg_resized.png", seg.x*gridSize, seg.y*gridSize, gridSize*2, gridSize*2);
      } else {
        ctx.beginPath(); ctx.arc(seg.x*gridSize+gridSize/2, seg.y*gridSize+gridSize/2, gridSize/2.2, 0,2*Math.PI);
        ctx.fillStyle=colors[level % colors.length]; ctx.fill();
      }
    });
  }

  function flashEffect() {
    flash.style.display="block"; flash.style.animation="none"; flash.offsetHeight;
  ...
  ["up","down","left","right"].forEach(id => {
    document.getElementById(id).addEventListener("touchstart", e => {
      ...
      handleDirection({ up:"ArrowUp", down:"ArrowDown", left:"ArrowLeft", right:"ArrowRight"}[id]);
    });
  });

  muteButton.addEventListener("click", () => {
    if (bgMusic.paused) {
      bgMusic.play(); muteButton.textContent="🔊";
    } else {
      bgMusic.pause(); muteButton.textContent="🔇";
    }
  });
